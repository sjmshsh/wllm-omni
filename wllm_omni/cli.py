...
def parse_args():
    ...
    parser.add_argument(
        "--stream",
        action="store_true",
        help="Stream AR text deltas. Currently supported only with --pipeline ar_text.",
    )
...
def main():
    ...
    if args.stream and pipeline != AR_TEXT_PIPELINE:
        raise ValueError("--stream currently supports only --pipeline ar_text.")
    llm = OmniLLM(
        args.model,
    ...
    if pipeline == AR_TEXT_PIPELINE:
        if args.stream:
            for step_output in llm.generate_ar_stream(args.prompt):
                if step_output.text_delta:
                    print(step_output.text_delta, end="", flush=True)
            print(flush=True)
